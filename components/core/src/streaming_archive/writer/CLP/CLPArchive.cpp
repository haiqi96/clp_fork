...
namespace streaming_archive::writer {
        ...
        for (uint32_t i = 1; i < uncompressed_msg_pos; i++) {
            ...
            switch (token_type) {
                ...
                case (int) compressor_frontend::SymbolID::TokenFloatId: {
                    encoded_variable_t encoded_var;
                    if (!EncodedVariableInterpreter::convert_string_to_representable_float_var(token.get_string(), encoded_var)) {
                        variable_dictionary_id_t id;
                        m_var_dict.add_entry(token.get_string(), id);
                        encoded_var = EncodedVariableInterpreter::encode_var_dict_id(id);
                        m_logtype_dict_entry.add_dictionary_var();
                                                              ...
                                                              ArrayBackedPosIntSet<variable_dictionary_id_t>& segment_var_ids)
    {
        ...
        m_local_metadata->increment_static_compressed_size(clp_segment.get_compressed_size());
    ...
    void CLPArchive::write_log_event_ir(ir::LogEvent<ffi::four_byte_encoded_variable_t> const& log_event) {
    ...
    void CLPArchive::write_log_event_ir(ir::LogEvent<ffi::eight_byte_encoded_variable_t> const& log_event) {
        std::vector<ffi::eight_byte_encoded_variable_t> encoded_vars;
        std::vector<variable_dictionary_id_t> var_ids;
        size_t original_num_bytes{0};
        EncodedVariableInterpreter::encode_and_add_to_dictionary(
