...
bool FileCompressor::compress_file(
    ...
    if (CommandLineArguments::InputSource::S3 == m_input_source) {
        ...
        buffered_file_reader = make_unique<BufferedFileReader>(make_unique<NetworkReader>(file_to_compress.get_path()));
        ...
        buffered_file_reader = make_unique<BufferedFileReader>(make_unique<FileReader>(file_to_compress.get_path()));
    }
